...
def generate_data4debug(data_dir, sample_num = 500):
    if ("small_" in data_dir) and os.path.exists(data_dir.replace("small_", '')):
        ...
        for s in ["dev", "train", "test"]:
            ...
            # os.system(f"head -n {sample_num} {ori_data_dir}/{s}_temp.jsonl > {data_dir}/{s}_temp.jsonl")

        #train_stat = 34985
        #s = "train"
        #sample_num = 1000
        #os.system(f"sed -n \'{train_stat},{train_stat+sample_num}p\' {ori_data_dir}/{s}.jsonl > {data_dir}/{s}.jsonl")
        #os.system(f"sed -n \'{train_stat},{train_stat+sample_num}p\' {ori_data_dir}/{s}_temp.jsonl > {data_dir}/{s}_temp.jsonl")
