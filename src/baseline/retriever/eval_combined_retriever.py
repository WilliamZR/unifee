...
def evidence_coverage(args):
    ...
    with open('{0}/{1}.roberta.not_precomputed.p{2}.s{3}.t{4}.jsonl'.format('data',
        ...
        for idx,line in enumerate(f): 
            ...
            docs_gold_t = set(
                [clean_title(ev.split('_')[0]) + '_table_' + ev.split('_')[3 if '_header_cell_' in ev else 2] for ev in
                 docs_gold_e if "_cell_" in ev])
            docs_gold = set(docs_gold_s | docs_gold_t)

            if len(docs_gold) == 0:
                continue
            docs_gold = set([clean_title(doc).strip() for doc in docs_gold])
            #predicted_sentences = [clean_title(ele[0]) + '_sentence_' + ele[1].split('_')[1] for ele in
            #                       js['predicted_evidence'] if ele[1].split('_')[0] == 'sentence']
            predicted_sentences = [ele for ele in js['predicted_evidence'] if '_sentence_' in ele]
            predicted_tables = [clean_title(ele[0]) + '_table_' + ele[1].split('_')[1] for ele in
                                js['predicted_evidence'] if ele[1].split('_')[0] == 'table']
