...
def claim_evidence_predictor(annotations_dev, args):
    ...
    claim_evidence_input_test = [(prepare_input(anno, 'schlichtkrull'), 'SUPPORTS') for anno in tqdm(annotations_dev)]


    text_test, labels_test = process_data(claim_evidence_input_test)


    tokenizer = RobertaTokenizer.from_pretrained('/home/hunan/bert_weights/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli', model_max_length = 512)
    ...
    #predictions_map = {i+1:predictions[i] for i,annota in enumerate(annotations_dev)}
...
def main():
    ...
    anno_processor_dev = AnnotationProcessor(args.input_path, has_content =False )
