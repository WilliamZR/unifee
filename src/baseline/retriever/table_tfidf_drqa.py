...
def tf_idf_sim(claim, lines,freqs=None):
    ...
    line_ids,scores = tfidf.closest_docs(claim,args.max_tabs)
    ret_lines = []
...
def tf_idf_claim(line):
    if 'predicted_pages' in line:
        ...
        for page in pages:
            ...
            p_lines.extend(zip(tables, [page] * len(lines), sentence_ids))
        ...
        line["predicted_tables"] = [(s["page"], s["line_on_page"]) for s in scores]
    return line
...
if __name__ == "__main__":
    ...
    with open("{0}/{1}.tables.not_precomputed.p5.t3.jsonl".format(args.data_path, args.split, args.max_page),"r") as f, open("{0}/{1}.tables.{4}.p{2}.t{3}.jsonl".format(args.data_path, args.split, args.max_page, args.max_tabs,"precomputed" if args.use_precomputed else "not_precomputed"), "w+") as out_file:
        lines = jlr.process(f)
