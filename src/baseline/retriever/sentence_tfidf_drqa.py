...
def tf_idf_sim(claim, lines,freqs=None):
    ...
    for idx,line in enumerate(line_ids):
        ret_lines.append(lines[line])##sentence
        ret_lines[-1]["score"] = scores[idx]## score of sentence
    return ret_lines
...
def tf_idf_claim(line):
    ...
    if 'predicted_pages' in line:
        ...
        pages = [p[0] for p in sorted_p[:args.max_page]] ## select top pages
        p_lines = []
        for page in pages:
            ...
            try:
                ...
                continue
            current_page = WikiPage(page, lines)## page_title,page_json
...
if __name__ == "__main__":
    ...
    with open("{0}/{1}.tables.not_precomputed.p5.t3.jsonl".format(args.data_path, args.split, args.max_page),"r") as f, open("{0}/{1}.sentences.{4}.p{2}.s{3}.jsonl".format(args.data_path, args.split, args.max_page, args.max_sent,"precomputed" if args.use_precomputed else "not_precomputed"), "w+") as out_file:
        lines = jlr.process(f)
        #print(f'lines:{lines}')
